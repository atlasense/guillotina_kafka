...
class StartConsumerCommand(ServerCommand):
    ...
    def get_parser(self):
        ...
        parser.add_argument(
            "--topics", nargs="*", help="Kafka topics to consume from", type=str
        )
        parser.add_argument(
            "--regex-topic",
            type=str,
        ...
        parser.add_argument("--within", type=int)
        return parser
    ...
    def get_consumer(self, arguments):
        ...
        if not worker:
            raise ConsumerWorkerLookupError("Worker has not been registered.")

        try:
            consumer_worker = resolve_dotted_name(worker["path"])
        except Exception:
            raise ConsumerWorkerLookupError("Worker has not been registered.")

        topic_prefix = app_settings["kafka"].get("topic_prefix")
        if topic_prefix:
            worker["topics"] = [f"{topic_prefix}{topic}" for topic in worker["topics"]]
        ...
        try:
            ...
            consumer = consumer_klas(
                ...
                group_id=(
                    arguments.consumer_group or worker.get("group", "default")
    ...
    def run(self, arguments, settings, app):
        ...
        loop = self.get_loop()
        asyncio.ensure_future(self.run_consumer(consumer, arguments), loop=loop)
